var TableDatatablesManaged = function () {
    var oTable;
    var initTable = function () {
        // begin: third table
        oTable = $('#myTable')
            .dataTable(
                {
                    "bProcessing" : true,
                    "bServerSide" : true,
                    "bFilter" : false,// 去掉搜索框
                    "sServerMethod" : "POST",
                    "bLengthChange":false,
                    "iDisplayLength" : 10,
                    "bAutoWidth" : false,
                    "jQueryUI" : false,
                    "ordering" : false,// 是否可以排序
                    "sAjaxSource" : ctx+"/nmis/api/applyStatus/statusList",
                    'fnServerParams' : function(aoData) {
                        // var orgName = $("#orgname").val();
                        // var patientName = $("#patientname").val();
                        // var expertName = $("#expertname").val();
                        // var sdate = $("#sdate").val();
                        // var edate = $("#edate").val();
                        // aoData.push({
                        //     "name": "conditions['orgName']",
                        //     "value": orgName
                        // },{
                        //     "name": "conditions['patientName']",
                        //     "value": patientName
                        // },{
                        //     "name": "conditions['expertName']",
                        //     "value": expertName
                        // },{
                        //     "name": "conditions['sdate']",
                        //     "value": sdate
                        // },{
                        //     "name": "conditions['edate']",
                        //     "value": edate
                        // });
                    },
                    "language": {
                        "aria": {
                            "sortAscending": ": activate to sort column ascending",
                            "sortDescending": ": activate to sort column descending"
                        },
                        "emptyTable": "无",
                        "info": "显示 _START_ - _END_ 共 _TOTAL_ 条记录",
                        "infoEmpty": "无记录",
                        "infoFiltered": "(filtered1 from _MAX_ total records)",
                        "lengthMenu": "Show _MENU_",
                        "search": "查找:",
                        "zeroRecords": "无记录",
                        "paginate": {
                            "previous":"上一页",
                            "next": "下一页",
                            "last": "最后",
                            "first": "首页"
                        }
                    },
                    "aoColumns" : [
                        { "data": "applyDatetime","sClass": "center"},
                        { "data": "userName","bSortable": false,"sClass": "center"},
                        { "data": "orgName","bSortable": false,"sClass": "center" },
                        { "data": "approveCompleted","bSortable": false,"sClass": "center" },
                        { "data": "certificatePrinted","bSortable": false,"sClass": "center" },
                        { "data": "userId","bSortable": false,"sClass": "center" },],
                    "createdRow" : function(row, data,
                                            index) {
                        var userId = data.userId;
                        var operlook = '<a href="javascript:TableDatatablesManaged.onLook(\''+userId +'\')" class="btn btn-xs btn-default"><i class="fa fa-folder-open-o"></i> '
                            + '查看' + ' </a>';
                        $('td', row).eq(5).addClass(
                            "hidden-480").html(
                            operlook);
                        var approveCompleted = data.approveCompleted;
                        var certificatePrinted = data.certificatePrinted;
                        if(approveCompleted == true){
                            $('td', row).eq(3).html("是");
                        }else if(approveCompleted == false){
                            $('td', row).eq(3).html("否");
                        }
                        if(certificatePrinted == true){
                            $('td', row).eq(4).html("是");
                        }else if(certificatePrinted == false){
                            $('td', row).eq(4).html("否");
                        }
                    }
                });

    }

    var onLook =  function onLook(userId){
        window.location.href=ctx+"/nmis/apply/examine/detail/"+userId;
    }

    return {
        init: function () {
            initTable();
        },
        onLook:function (userId) {
            onLook(userId);
        }
    };

}();
jQuery(document).ready(function() {
    TableDatatablesManaged.init();
});
