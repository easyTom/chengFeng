var registrationSubject = function () {
    ...
    var handleSubjectDelete = function (id) {
        request("/nmis/api/apply/registrationSubject/delete?id=" + id,"post",null,function () {
            registrationSubject.init();
        });
    }
    ...
    var getTemplate = function(subject) {
        return '<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">\n' +
            ...
            '\t\t\t\t\t\t\t\t\t</div>\n' +
            '\t\t\t\t\t\t\t\t\t<div class="icon">\n' +
            '\t\t\t\t\t\t\t\t\t\t<a class="fa fa-close" onclick="registrationSubject.deleteSubject(\''+ subject.id + '\')"></a>\n' +
            '\t\t\t\t\t\t\t\t\t</div>\n' +
            '\t\t\t\t\t\t\t\t</div>\n' +
            '\t\t\t\t\t\t\t</div>\n' +
            '\t\t\t\t\t\t</div>';
    ...
    var handleInitView = function(userId){
        request("/nmis/api/apply/registrationSubject?userId="+ userId,"get",'',function(response){
        ...
        request("/nmis/api/apply/hospitalSubject?userId="+ userId,"get",'',function(response){
            ...
            for(var key in response.data){
                var value = response.data[key];
                select2.append(`<option value="${value.subject}">${hospitalSubject_dic.getHospitalSubjectNameByCode(value.subject)}</option>`);
            }
        });
    }
    ...
    var handleSubjectAdd = function (userId,subjects){
        ...
        request('/nmis/api/apply/registrationSubject','post',fd,function(res){
            registrationSubject.init();
        })
    }
